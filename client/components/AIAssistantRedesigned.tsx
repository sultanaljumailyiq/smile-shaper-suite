...
import { MessageCircle, X, Send, Sparkles, Brain, Stethoscope, Calendar, FileText, Mic, Image, Minimize2, Maximize2, Bot, Zap, Eye, Activity, Shield, Clock, TrendingUp, Heart, Camera, Upload, Settings, HelpCircle, Star, Volume2, VolumeX, RotateCcw, Copy, Download, Share2, BookOpen, Lightbulb, Target, Award, Cpu, Database, Wifi, WifiOff, ChevronRight, Palette } from "lucide-react";
...
interface Message {
  ...
  sender: "user" | "ai";
  timestamp: Date;
  type?: "text" | "suggestion" | "analysis" | "diagnosis" | "treatment" | "urgent";
  confidence?: number;
  attachments?: string[];
  metadata?: {
...
const AIAssistantRedesigned = () => {
  ...
  const [currentMode, setCurrentMode] = useState<"chat" | "analysis" | "planning">("chat");
  const [isListening, setIsListening] = useState(false);
  const [isSoundEnabled, setIsSoundEnabled] = useState(true);
  const [isConnected, setIsConnected] = useState(true);
  const [aiStatus, setAiStatus] = useState<"ready" | "thinking" | "analyzing" | "offline">("ready");
...
كيف يمكنني مساعدتك اليوم؟`,
  ...
  const generateAIResponse = (userInput: string): Message => {
    ...
    if (lowerInput.includes("أشعة") || lowerInput.includes("تحليل") || lowerInput.includes("صورة")) {
      type = "diagnosis";
        ...
        setShowSuggestions(true); // إظهار الاقتراحات عند فتح النافذة
          ...
          <div className="relative flex items-center gap-3">
            ...
            <div className="flex flex-col items-center gap-1">
              <div className={cn("w-3 h-3 rounded-full animate-pulse shadow-lg", getStatusColor())}></div>
      ...
      <div className={cn("bg-white/95 backdrop-blur-2xl rounded-3xl shadow-2xl border border-gray-200/50 transition-all duration-500", isMinimized ? "w-80 h-20" : isExpanded ? "w-[600px] h-[850px]" : "w-[500px] h-[750px]", "overflow-hidden ring-1 ring-black/5")}>
        ...
        <div className="relative z-10 flex items-center justify-between p-5 border-b border-gray-200/50 bg-gradient-to-r from-blue-50 via-purple-50 to-indigo-50">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setIsMinimized(!isMinimized)} className="p-2 hover:bg-gray-100 rounded-xl transition-colors" title={isMinimized ? "توسيع" : "تصغير"}>
              {isMinimized ? <Maximize2 className="w-4 h-4 text-gray-600" /> : <Minimize2 className="w-4 h-4 text-gray-600" />}
            </button>
            <button onClick={() => setIsOpen(false)} className="p-2 hover:bg-red-50 rounded-xl transition-colors group">
        ...
        {!isMinimized && <>
            ...
            {showSuggestions && <div className="p-3 sm:p-5 border-b border-gray-100 bg-gradient-to-r from-gray-50 via-blue-50 to-purple-50">
                <div className="flex items-center justify-between mb-3 sm:mb-4">
                  ...
                  <button onClick={() => setShowSuggestions(false)} className="p-1 hover:bg-gray-200 rounded-lg transition-colors">
                    <X className="w-4 h-4 text-gray-500" />
                  </button>
                </div>

                <div className="grid grid-cols-2 gap-2 sm:gap-3">
                  {predefinedQuestions.slice(0, 6).map((question, index) => <button key={index} onClick={() => handleQuestionClick(question.text)} className="group p-2 sm:p-4 bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-200/50 hover:border-blue-300 hover:bg-blue-50/80 transition-all duration-300 text-right hover:scale-[1.02] hover:shadow-lg">
                      <div className="flex items-start gap-2 sm:gap-3">
                        <div className="w-6 h-6 sm:w-10 sm:h-10 bg-gradient-to-r from-blue-500 to-purple-500 rounded-xl flex items-center justify-center text-white text-sm sm:text-lg flex-shrink-0 group-hover:scale-110 transition-transform">
                          {question.icon}
                        </div>
                ...
                return <button key={action.id} onClick={() => {
                  handleSendMessage(action.prompt);
                }} className={cn("flex flex-col items-center gap-0.5 p-1.5 sm:p-2 rounded-lg transition-all hover:scale-105 transform active:scale-95", action.color, "text-white shadow-md hover:shadow-lg border border-white/20", "hover:border-white/40 focus:outline-none focus:ring-1 focus:ring-white/50")}>
            ...
            {/* منطقة الرسائل */}
            <div className={cn("flex-1 overflow-y-auto p-3 sm:p-5 space-y-3 sm:space-y-4 scroll-smooth", "scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100", showSuggestions ? isExpanded ? "h-[300px]" : "h-[250px]" : isExpanded ? "h-[550px]" : "h-[450px]", "max-h-[60vh] min-h-[200px]")}>
              {messages.map(message => <div key={message.id} className={cn("flex", message.sender === "user" ? "justify-end" : "justify-start")}>
                  <div className={cn("max-w-[85%] rounded-2xl px-3 sm:px-5 py-3 sm:py-4 text-xs sm:text-sm relative", "transition-all duration-200 hover:shadow-md", message.sender === "user" ? "bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg hover:shadow-xl" : "bg-gradient-to-r from-gray-50 to-blue-50 text-gray-900 border border-gray-200 shadow-sm hover:border-blue-300", "max-w-[90%] sm:max-w-[85%]")}>
                    {message.sender === "ai" && <div className="flex items-center gap-2 mb-2 sm:mb-3 pb-2 border-b border-gray-200">
                        ...
                        {message.confidence && <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full">
              ...
              <div className="px-5 pb-4 border-t border-gray-200/30">
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-3 text-gray-600">
                    ...
                    {isConnected && <div className="flex items-center gap-1 text-green-600">
