        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
              {/* Notifications Card */}
              <Link to="/dentist-hub/notifications" className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md transition-shadow">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-gray-900">عرض الكل</p>
                  </div>
                </div>
              </Link>

              {/* Notes Card */}
              <Link to="/dentist-hub?section=notes" className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md transition-shadow">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-gray-900">إدارة</p>
                  </div>
                </div>
              </Link>

              {/* Clinics Summary Card */}
              <Link to="/dentist-hub/clinics" className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md transition-shadow">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-blue-100 flex items-center justify-center">
                    <Building className="w-5 h-5 text-blue-600" />
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">ملخص العيادات</p>
                    <p className="text-lg font-bold text-gray-900">
                      {clinicsCount !== null ? `${clinicsCount} عيادا��` : "عرض الملخص"}
                    </p>
                  </div>
                </div>
              </Link>
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 mb-6">
              {Object.entries(sectionConfig).map(([key, section]) => <button key={key} onClick={() => setSelectedSection(key as SectionType)} className={cn("flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors", selectedSection === key ? `bg-gradient-to-r ${section.gradient} text-white` : "bg-white text-gray-700 hover:bg-gray-100")}>
            ...
            <div className="relative max-w-md">
              ...
              <input type="text" placeholder="البحث في الم��ضلات..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
          </div>

          {/* Content */}
          {filteredItems.length > 0 ? <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {filteredItems.map(renderCard)}
            </div> : <div className="text-center py-12">
              <div className={cn("w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4", `bg-gradient-to-r ${config.gradient}`)}>
                <config.icon className="w-8 h-8 text-white" />
              </div>
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                لا توجد مف��لات في {config.title.replace("مفضلاتي - ", "")}
              </h3>
              <p className="text-gray-600 mb-6">
                ابدأ في إضافة العناصر لمفضلاتك لتراها هنا
              </p>
              <Link to={selectedSection === "marketplace" ? "/dental-supply" : selectedSection === "community" ? "/community" : selectedSection === "jobs" ? "/jobs" : selectedSection === "education" ? "/education" : "/"} className={cn("inline-flex items-center gap-2 px-6 py-3 rounded-xl text-white font-medium transition-colors", `bg-gradient-to-r ${config.gradient} hover:opacity-90`)}>
