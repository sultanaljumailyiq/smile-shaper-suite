...
export default function DentistHub() {
  ...
  const FavoritesSection = () => <div className="bg-white rounded-2xl border border-gray-100 p-4 md:p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {favorites.slice(0, 4).map(f => <Link key={f.id} to={`/dental-supply/product/${f.id}`} className="p-3 border rounded-xl hover:bg-gray-50">
            <div className="font-medium text-gray-900 truncate">
              {f.arabicName || f.name}
            </div>
            <div className="text-xs text-gray-600 mt-1">{f.brand}</div>
          </Link>)}
        {favorites.length === 0 && <div className="text-sm text-gray-500">لا توجد عناصر ف�� المفضلة</div>}
      </div>
    </div>;
  ...
  return <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 with-floating-nav" dir="rtl">
      ...
      <div className="pt-2">
        <main className="p-3 sm:p-6 lg:p-8">
          {location.pathname.startsWith("/dentist-hub/clinics") ? <AdvancedClinicManagement /> : location.pathname.startsWith("/dentist-hub/notifications") ? <UnifiedNotifications /> : location.pathname.startsWith("/dentist-hub/profile") ? <IntegratedDentistProfile /> : location.pathname.startsWith("/dentist-hub/favorites") ? <Favorites /> : location.pathname.startsWith("/dentist-hub/smart-clinic") ? <SmartClinic /> : <>
